...
export function PromptInputProvider({
  ...
  const remove = useCallback((id: string) => {
    setAttachements((prev) => {
      ...
      if (found?.url) {
        console.log("Revoking URL for", 2);
  ...
  const clear = useCallback(() => {
    setAttachements((prev) => {
      for (const f of prev) {
        if (f.url) {
          console.log("Revoking URL for", 3);
  ...
  return (
    <PromptInputHoverCard>
      <HoverCardTrigger asChild>
        <div
          className={cn(
            "group relative max-w-full overflow-hidden flex h-8 cursor-default select-none items-center gap-1.5 rounded-md border border-border px-1.5 font-medium text-sm transition-all hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50",
            className
          )}
        >
  ...
  const remove = usingProvider
    ...
    : (id: string) =>
        setItems((prev) => {
          ...
          if (found?.url) {
            console.log("Revoking URL for", 4);
            URL.revokeObjectURL(found.url);
          }
    ...
    : () =>
        setItems((prev) => {
          for (const file of prev) {
            if (file.url) {
              console.log("Revoking URL for", 5);
    ...
    () => () => {
      if (!usingProvider) {
        for (const f of files) {
          console.log("Revoking URL for", 1);
...
export const PromptInputSubmit = ({
  ...
  let Icon = <CornerDownLeftIcon className="size-4" color="white" />;
